...
Scope was also added to incorporate a new navigation link for 'Compare Plans', but the url was not provided. This link was added into the app but currently does not navigate anywhere, similar to the other links. One the url is availabl, a hotfix can be implemented to link it. Go.
- Inject #2 (Wed): Ops alerted us to a Prod bug with the quoting of Renter's coverage, which was related to a failing CI run due to a missing rating file for Renter's insurance. Development team has been advised to ensure that file is accessible and to prioritize the bug fix, but this new functionality release will not be impacted. Go.

## Slide 5 — What I'd do differently next round

- I would do things the same way if these scenarios happened again
- I would have prefered to have all requirements and links available to implement for the scope creep asks, so maybe there would be some push-back on that ask.
